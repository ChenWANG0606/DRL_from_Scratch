...
4. 同策略on-policy与异策略off-policy
   ...
   - 基于Q值 ！= 一定异策略
     - SARSA就是同策略的
   - 为什么on-policy只能用自己的数据，off-policy可以用不同策略数据
     - 因为on-policy本身在更新策略梯度，而策略梯度依赖某个策略参数生成，一旦策略更新，收集到的轨迹就会和当前策略不匹配了
     - off-policy实在更新对每个动作Q值的估计，而Q网络不依赖任何策略，只是评估一段特定轨迹的价值，因此只要轨迹符合Q学习公式（argmax），都可以作为训练数据，训练数据越多样反而越好
